...
Gateway 仍然保留，因为它负责首帧令牌/账户身份校验、幂等、单 writer、查询 singleflight、可靠事件投递和 ACK；外置策略不得直接写 Helper 队列。TCP 只允许绑定 `127.0.0.1`，项目不会创建 Windows 入站防火墙规则。
...
- [大 QMT 内置 Python](大QMT内置python/README.md)：Helper、loader、生成器和 20 项回归；
