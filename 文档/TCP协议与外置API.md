...
```powershell
python .\示例\raw_tcp_query.py --query-type ACCOUNT_STATUS
```
...
```powershell
python .\示例\query_account.py --query-type ACCOUNT_STATUS
```

可靠回调消费者：

```powershell
python .\示例\callback_client.py --query-on-start ACCOUNT_STATUS
```

下单 dry-run，不打开网络：

```powershell
python .\示例\async_order.py `
...
撤单同样默认 dry-run，live 短语见 `python .\示例\async_cancel.py --help`。
